...
@php
    $classes = $tweet ?? false ? 'item-start xl:w-60 bg-[#1DA1F2] rounded-full justify-center' : 'flex flex-col items-start justify-center transition duration-300';
@endphp

@if ($title)
    <div {{ $attributes->merge(['class' => $classes]) }}>
        <a href="{{ $route }}"
            class="flex items-center p-2 transition-all rounded-full xl:justify-center hover:bg-gray-600/20 xl:px-4 xl:py-3">
